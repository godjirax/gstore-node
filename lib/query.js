    ...
    initQuery(namespace, transaction) {
        ...
        query.run = function runQuery(options = {}, responseHandler = res => res) {
            ...
            function addKeys(data) {
              return new Promise(function test2(resolve) {
                if (options.format === 'ENTITY') {
                  const newData = data[0].map(function test3(d) {
                    d[Model.gstore.ds.KEY] = Model.key(
                      d[Model.gstore.ds.KEY].id
                    );
                    return d;
                  });
                  resolve([newData, data[1]]);
                  return;
                }
